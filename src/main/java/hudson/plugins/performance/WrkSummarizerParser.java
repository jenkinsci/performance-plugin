 ...
 * 
 * <p>
 * Note that Wrk does not produce request-level data, and can only be
 * processed in it's summarized form (unless extended to do otherwise).
 * 
...
public class WrkSummarizerParser extends PerformanceReportParser {
  ...
  @DataBoundConstructor
	...
	public Collection<PerformanceReport> parse(AbstractBuild<?, ?> build,
		...
		List<PerformanceReport> result = new ArrayList<PerformanceReport>(reports.size());
  			...
  			while (s.hasNextLine()) {
  				...
  				try {
  					...
  					case LATENCY_DIST:
  					  ...
  					  long latencyAvg = getTime(latencyScanner.next(), logger, TimeUnit.MILLISECOND);
  					  latencyScanner.next(); // stdDev (skipping)
  					  long latencyMax = getTime(latencyScanner.next(), logger, TimeUnit.MILLISECOND);
  					...
  					case SUMMARY:
  						...
  						summaryScanner.next(); summaryScanner.next(); summaryScanner.next();
//  						long totalTime = getTime(summaryScanner.next(), logger, TimeUnit.SECOND);
  ...
  /**
	 * Given a time string (eg: 0ms, 1m, 2s, 3h, etc.) parse and yield the time
	 * in a specified time unit (millisecond, second, minute, hour)
	 * 
	 * <p>
	 ...
	 * @param logger      Logger taken from the TaskListener
    ...
    double factor = 0;
		...
		/*
		 * Calculate 'factor' so that we can get the input time in ms
		 * (eg: 5m = 300000, 3s = 3000)
		 */
	...
	public LineType determineLineType(String t1, String t2) {
		if (t1.equals("running")) {
		  ...
		  if (t2.equals("distribution")) return LineType.LATENCY_DIST_BUCKET_HEADER;
		  else return LineType.LATENCY_DIST;
		}
